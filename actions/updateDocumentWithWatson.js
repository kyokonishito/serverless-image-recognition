    ...
    return new Promise(function(resolve, reject) {
        ...
        mydb.attachment.get(params.id, 'image', function(err, data) {
            if (err) {
                ...
                resolve(getLang(params, data,params.id,params.WATSON_VR_APIKEY)); //changed
            }
        });
    });
}
...
function processImageToWatson(data,id,apikey, lang) { //added
...
function getLang(params, imgdata,id,apikey){
    return new Promise(function(resolve, reject) {
        let mydb = cloudant.db.use(dbName);
        mydb.get(id,  function(err, data) {
            if (err) {
                reject(err);
            } else {
                console.log(params)
                console.log("LANG:"+JSON.stringify(data.lang));
                resolve(processImageToWatson(imgdata,id,apikey,data.lang));
            }
        });
    });
